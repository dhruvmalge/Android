...
import 'package:music_streaming/storage/local_storage_file_loader.dart';
...
class _CategorySongsScreenState extends State<CategorySongsScreen> {
  final String _baseUrl = 'https://7822ae7d7b1d.ngrok-free.app';
  List<Song> _songs = [];
  ...
  Future<void> _fetchSongs({String categoryId = 'all'}) async {
    ...
    try {
      if (categoryId == 'local') {
        final localSongs = await loadSongsFromLocalStorage();

        setState(() {
          _songs = localSongs.map((local) => Song(
            id: local.path.hashCode, // Use path hash as a unique ID
            title: local.title,
            artist: local.artist,
            tags: ['local'],
          )).toList();
